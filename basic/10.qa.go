...
Q-1：和跟 Redis 相比，SimpleKV 还缺少什么？
A：10.qa_1.jpg

Q-2：整数数组和压缩列表作为底层数据结构的优势是什么？
A：整数数组和压缩列表的设计，充分体现了 Redis“又快又省”特点中的“省”，也就是节省内存空间
...
Q-3：Redis 基本 IO 模型中还有哪些潜在的性能瓶颈？
A：进一步理解阻塞操作对 Redis 单线程性能的影响
	在 Redis 基本 IO 模型中，主要是主线程在执行操作，任何耗时的操作，例如 bigkey、全量返回等操作，都是潜在的性能瓶颈

Q-4.1：AOF 重写过程中有没有其他潜在的阻塞风险？
A：两个风险
	1.阻塞风险一
		Redis 主线程 fork 创建 bgrewriteaof 子进程时，内核需要创建用于管理子进程的相关数据结构
		这些数据结构在操作系统中通常叫作进程控制块（Process ControlBlock，简称为 PCB）
		内核要把主线程的 PCB 内容拷贝给子进程
		这个创建和拷贝过程由内核执行，是会阻塞主线程的。而且，在拷贝过程中，子进程要拷贝父进程的页表，这个过程的耗时和 Redis 实例的内存大小有关
		如果 Redis 实例内存大，页表就会大，fork执行时间就会长，这就会给主线程带来阻塞风险
	2.阻塞风险二
		bgrewriteaof 子进程会和主线程共享内存
		当主线程收到新写或修改的操作时，主线程会申请新的内存空间，用来保存新写或修改的数据
		如果操作的是 bigkey，也就是数据量大的集合类型数据，那么，主线程会因为申请大空间而面临阻塞风险
		因为操作系统在分配内存空间时，有查找和锁的开销，这就会导致阻塞
Q-4.2：AOF 重写为什么不共享使用 AOF 本身的日志？
A：如果都用 AOF 日志的话，主线程要写，bgrewriteaof 子进程也要写，这两者会竞争文件系统的锁
	这就会对 Redis 主线程的性能造成影响

Q-5：使用一个 2 核 CPU、4GB 内存、500GB 磁盘的云主机运行 Redis，Redis 数据库的数据量大小差不多是 2GB
	当时 Redis 主要以修改操作为主，写读比例差不多在 8:2 左右，也就是说，如果有 100 个请求，80 个请求执行的是修改操作
	在这个场景下，用 RDB 做持久化有什么风险吗？
A：两个风险
	1.内存不足的风险
		写时复制：
			Redis fork 一个 bgsave 子进程进行 RDB 写入，如果主线程再接收到写操作，就会采用写时复制
			写时复制需要给写操作的数据分配新的内存空间
			本问题中写的比例为 80%，那么，在持久化过程中，为了保存 80% 写操作涉及的数据，写时复制机制会在实例内存中，为这些数据再分配新内存空间
			分配的内存量相当于整个实例数据量的 80%，大约是 1.6GB，这样一来，整个系统内存的使用量就接近饱和了
			此时，如果实例还有大量的新 key 写入或 key 修改，云主机内存很快就会被吃光
		Swap 机制：
			如果云主机开启了Swap 机制，就会有一部分数据被换到磁盘上，当访问磁盘上的这部分数据时，性能会急剧下降
			如果云主机没有开启 Swap，会直接触发 OOM，整个 Redis 实例会面临被系统 kill 掉的风险
	2.主线程和子线程竞争使用 CPU 的风险
		生成 RDB 的子进程需要 CPU 核运行，主线程本身也需要 CPU 核运行
		而且，如果 Redis 还启用了后台线程，此时，主线程、子进程和后台线程都会竞争 CPU 资源
		由于云主机只有 2 核 CPU，这就会影响到主线程处理请求的速度

Q-6：为什么主从库间的复制不使用 AOF？
A：两个原因
	RDB 文件是二进制文件，无论是要把 RDB 写入磁盘，还是要通过网络传输 RDB，IO 效率都比记录和传输 AOF 的高
	在从库端进行恢复时，用 RDB 的恢复效率要高于用 AOF

Q-7.1：在主从切换过程中，客户端能否正常地进行请求操作呢？
A：主从集群一般是采用读写分离模式，当主库故障后，客户端仍然可以把读请求发送给从库，让从库服务
	但是，对于写请求操作，客户端就无法执行了
Q-7.2：如果想要应用程序不感知服务的中断，还需要哨兵或客户端再做些什么吗？
A：两方面
	1.客户端需要能缓存应用发送的写请求
		只要不是同步写操作（Redis 应用场景一般也没有同步写），写请求通常不会在应用程序的关键路径上
		所以，客户端缓存写请求后，给应用程序返回一个确认就行
	2.主从切换完成后，客户端要能和新主库重新建立连接，哨兵需要提供订阅频道，让客户端能够订阅到新主库的信息
		同时，客户端也需要能主动和哨兵通信，询问新主库的信息

Q-8.1：5 个哨兵实例的集群，quorum 值设为 2
	在运行过程中，如果有 3 个哨兵实例都发生故障了，此时，Redis 主库如果有故障，还能正确地判断主库“客观下线”吗
	如果可以的话，还能进行主从库自动切换吗？
A：因为判定主库“客观下线”的依据是，认为主库“主观下线”的哨兵个数要大于等于 quorum 值
...
Q-8.2：哨兵实例是不是越多越好呢？如果同时调大 down-after-milliseconds 值，对减少误判是不是也有好处？
A：哨兵实例越多，误判率会越低
	但是在判定主库下线和选举 Leader 时，实例需要拿到的赞成票数也越多，等待所有哨兵投完票的时间可能也会相应增加，主从库切换的时间也会变长
	客户端容易堆积较多的请求操作，可能会导致客户端请求溢出，从而造成请求丢失
	如果业务层对 Redis 的操作有响应时间要求，就可能会因为新主库一直没有选定，新操作无法执行而发生超时报警

	调大 down-after-milliseconds 后，可能会导致这样的情况：
	主库实际已经发生故障了，但是哨兵过了很长时间才判断出来，这就会影响到 Redis 对业务的可用性

Q-9：为什么 Redis 不直接用一个表，把键值对和实例的对应关系记录下来？
A：如果使用表记录键值对和实例的对应关系，一旦键值对和实例的对应关系发生了变化（例如实例有增减或者数据重新分布），就要修改表
...
1.rehash 的触发时机和渐进式执行机制
	1.1.Redis 什么时候做 rehash？
		...
		第一种情况：
			...
			如果此时，Redis 没有在生成 RDB 和重写 AOF，那么，就可以进行 rehash
			否则的话，再有数据写入时，哈希表就要开始使用查询较慢的链式哈希了
		第二种情况：
			装载因子大于等于 5 时，就表明当前保存的数据量已经远远大于哈希桶的个数，哈希桶里会有大量的链式哈希存在，性能会受到严重影响
			此时，就立马开始做 rehash
		不会 rehash
			如果装载因子小于 1，或者装载因子大于 1 但是小于 5，同时哈希表暂时不被允许进行 rehash（例如，实例正在生成 RDB 或者重写 AOF）
			此时，哈希表是不会进行 rehash 操作的
	1.2.采用渐进式 hash 时，如果实例暂时没有收到新请求，是不是就不做 rehash 了？
		不是
...
2.主线程、子进程和后台线程的联系与区别
	...
	主进程和主线程
		...
		举例：一个 C 程序片段，main 函数会直接调用一个 worker 函数，函数worker 就是执行一个 for 循环计算
			int counter = 0;
			void *worker() {
				for (int i=0;i<10;i++) {
					counter++;
			...
			这个程序运行后，它自己就是一个主进程，同时也是个主线程
		Redis
			Redis 启动以后，本身就是一个进程，它会接收客户端发送的请求，并处理读写操作请求
			而且，接收请求和处理请求操作是 Redis 的主要工作，Redis 没有再依赖于其他线程
			所以，一般可以把完成这个主要工作的 Redis 进程，称为主进程或主线程
	图示
		10.qa_fork_thread.jpg
	...
	fork 创建的子进程有哪些
		...
		bgrewriteaof 子进程
	Redis 使用的线程
		从 4.0 版本开始，Redis 也开始使用pthread_create 创建线程，这些线程在创建后，一般会自行执行一些任务
		例如执行异步删除任务
...
3.写时复制的底层实现机制
	...
	示例
		...
		那么，主线程就需要新分配一个物理页（假设是物理页 53），然后把修改后的虚页 7 里的数据写到物理页 53 上
		而虚页 7 里原来的数据仍然保存在物理页 33 上
...
4.replication buffer 和 repl_backlog_buffer 的区别
	...
	replication buffer
		Redis 主从库在进行复制时，当主库要把全量复制期间的写操作命令发给从库时，主库会先创建一个客户端，用来连接从库
		然后通过这个客户端，把写操作命令发给从库
	...
	repl_backlog_buffer
		一块专用 buffer，在 Redis 服务器启动后，开始一直接收写操作命令，这是所有从库共享的
		主库和从库会各自记录自己的复制进度
